...
def callback2(data):
    ...
    # rospy.loginfo("MD: %s", data.data)

def callback3(data):
    global rec_info

    rec_info = data.data
    # rospy.loginfo("rec: %s", data.data)
...
def receive_message():
    ...
    rospy.Subscriber('metal_detection', Int32, callback2, queue_size=1)
    rospy.Subscriber('reciever', array, callback3, queue_size=1)
    rospy.spin()
