...
class GPSNav:
    def __init__(self):
        ...
        self.mpu_sub = rospy.Subscriber('/compass', Float32, self.compass_callback)

        self.auto_control_sub = rospy.Subscriber('/gps_mode', Bool, self.turn_and_move_robot)
    ...
    def turn_and_move_robot(self, data):
        self.mode = data.data
        try:
            if self.mode and self.dist_to_travel > self.waypoint_accuracy:
                ...
                while abs(self.desired_angle - self.current_angle) > 0.5 and self.mode:
                    ...
                    if error > 0:
                        self.twist_pub.publish(Twist(linear=Vector3(0, 0, 0), angular=Vector3(0, 0, -0.25)))
                    else:
                        self.twist_pub.publish(Twist(linear=Vector3(0, 0, 0), angular=Vector3(0, 0, 0.25)))
                    if not self.mode:
                        break
                ...
                self.twist_pub.publish(Twist(linear=Vector3(0.2, 0, 0), angular=Vector3(0, 0, 0)))
